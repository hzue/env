...
function _ls() {
    LANG=zh_TW.BIG5
    /bin/ls -C -G $@ #| /usr/bin/iconv -f big5 -t utf8
    LANG=zh_TW.UTF-8
}


export GOPATH="$HOME/go"
export PATH="$PATH:$GOROOT/bin:$GOPATH/bin:$HOME/bin:/usr/local/go/bin"

if [ -f ~/.fzf.bash ]; then
    source ~/.fzf.bash
    export FZF_DEFAULT_OPTS="--no-mouse --reverse --exact --border --height 35%"
fi
